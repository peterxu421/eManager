...
public class VenueBooking_VenueWeekView extends Composite {
	...
	// an arrayList containing the the dates of the seven days in the week shown
	...
	public VenueBooking_VenueWeekView(Composite parent, int style,
		...
		weekViewTable.setBackground(SWTResourceManager
		...
		selectedCellColor = weekViewTable.getDisplay().getSystemColor(
				SWT.COLOR_LIST_SELECTION);
		notAvailableCellColor = weekViewTable.getDisplay().getSystemColor(
				SWT.COLOR_RED);
		...
		lblEnterPreferredEvent.setBackground(SWTResourceManager
				.getColor(SWT.COLOR_WHITE));
		lblEnterPreferredEvent.setFont(SWTResourceManager.getFont("Segoe UI",
				11, SWT.NORMAL));
		...
		lblSelectedDateAnd.setFont(SWTResourceManager.getFont("Segoe UI", 11,
				SWT.NORMAL));
		...
		lblNotePleaseSelect.setFont(SWTResourceManager.getFont("Courier New",
				9, SWT.NORMAL));
		lblNotePleaseSelect.setBounds(10, 424, 630, 62);
		toolkit.adapt(lblNotePleaseSelect, true, true);
		lblNotePleaseSelect
				.setText("Note: 1. Double click a cell to select the corresponding time slot.\n"
	...
	public class checkBookingInfo implements ICalendarListener {
		...
		public void popupClosed() {
			/*
			 * Clear the week view time table before start a new bookingInfo
			 * check
			 */
			...
			for (int i = 0; i < 7; i++) {
				dateInAWeekList.add(new Date(0, 0, 0)); // initialize the
														// arraylist
			}
			for (int i = 3; i < 7; i++) { // right hand side half
				day_of_week = calendar.get(Calendar.DAY_OF_WEEK);
				tblclmnTextFill(day_of_week, calendar, tblclmn[i]);
				_date = new Date(calendar.get(Calendar.YEAR),
						calendar.get(Calendar.MONTH) + 1,
						calendar.get(Calendar.DAY_OF_MONTH)); // MONTH+1 because
																// MONTH starts
																// from 0 in
																// default
			...
			calendar.add(Calendar.DAY_OF_MONTH, -4); // roll back to the
														// selected date
			for (int i = 3; i >= 0; i--) {
				day_of_week = calendar.get(Calendar.DAY_OF_WEEK);
				tblclmnTextFill(day_of_week, calendar, tblclmn[i]);
				_date = new Date(calendar.get(Calendar.YEAR),
						...
						calendar.get(Calendar.DAY_OF_MONTH)); // MONTH+1 because
																...
																// default
				dateInAWeekList.set(i, _date);
				calendar.add(Calendar.DAY_OF_MONTH, -1); // roll the date
															// backward
			}

			/*
			 * Update the table color highlighting from the list of selected
			 * date and time
			 */
			for (int k = 0; k < selectedDateTimeList.getItemCount(); k++) {
				...
				for (int i = 0; i < weekViewTable.getItemCount(); i++) {
					...
					if (Time.parseHour(item.getText(0)) == dt.getTimeStart()
							.getHour()) { // locate the time row
						for (int j = 0; j < dateInAWeekList.size(); j++) {
							if (dateInAWeekList.get(j).isEqualTo(dt.getDate())) { // locate
																					// the
																					// date
																					// column
			...
			ArrayList<VenueBookingApplication> bookingInfoList = db
					.getVenueBookingInfo(selectedVenue);
			if (!bookingInfoList.isEmpty()) {
				for (int i = 0; i < bookingInfoList.size(); i++) {
					if (bookingInfoList.get(i).getStatus() == MACRO.PENDING
							|| bookingInfoList.get(i).getStatus() == MACRO.APPROVED) {
						BookedDateTime dateTime = bookingInfoList.get(i)
								.getDateTime();
						for (int j = 0; j < dateInAWeekList.size(); j++) {
							if (dateInAWeekList.get(j).isEqualTo(
									dateTime.getDate())) { // find the booked
															// date
								for (int k = 0; k < weekViewTable
										.getItemCount(); k++) {
									if (Time.parseHour(weekViewTable.getItem(k)
											.getText(0)) == dateTime
											.getTimeStart().getHour()) { // find
																			// the
																			// booked
																			// time
										for (int t = 0; t < (dateTime
												.getTimeEnd().getHour() - dateTime
												.getTimeStart().getHour()); t++) {
											weekViewTable.getItem(k).setText(
													j + 1, "Booked");
											weekViewTable
													.getItem(k)
													.setBackground(j + 1,
															notAvailableCellColor);
											k++; // move to the next hour in the
													// week view time table
										}
									}
								}
							}
		...
		public void tblclmnTextFill(int day_of_week, Calendar calendar,
				TableColumn tblclmn) {
			switch (day_of_week) {
			case 1:
				tblclmn.setText("Sun " + (calendar.get(Calendar.MONTH) + 1)
						+ "/" + calendar.get(Calendar.DAY_OF_MONTH));
				break;
			case 2:
				tblclmn.setText("Mon " + (calendar.get(Calendar.MONTH) + 1)
						+ "/" + calendar.get(Calendar.DAY_OF_MONTH));
				break;
			case 3:
				tblclmn.setText("Tue " + (calendar.get(Calendar.MONTH) + 1)
						+ "/" + calendar.get(Calendar.DAY_OF_MONTH));
				break;
			case 4:
				tblclmn.setText("Wed " + (calendar.get(Calendar.MONTH) + 1)
						+ "/" + calendar.get(Calendar.DAY_OF_MONTH));
				break;
			case 5:
				tblclmn.setText("Thu " + (calendar.get(Calendar.MONTH) + 1)
						+ "/" + calendar.get(Calendar.DAY_OF_MONTH));
				break;
			case 6:
				tblclmn.setText("Fri " + (calendar.get(Calendar.MONTH) + 1)
						+ "/" + calendar.get(Calendar.DAY_OF_MONTH));
				break;
			case 7:
				tblclmn.setText("Sat " + (calendar.get(Calendar.MONTH) + 1)
						+ "/" + calendar.get(Calendar.DAY_OF_MONTH));
				break;
			}
	...
	public class chooseTime implements Listener {
		public void handleEvent(Event event) {
			...
			for (int i = 0; i < weekViewTable.getColumnCount(); i++) {
				...
				if (rect.contains(pt) && item.getText(i).isEmpty()
						&& !item.getBackground(i).equals(selectedCellColor)) { // choose
																				// only
																				// the
																				// empty
																				// time
					...
					weekViewTable.deselectAll();
					Time selectedTimeStart = new Time(Time.parseHour(item
							.getText(0)), 0, 0); // find the selected timeStart
					Time selectedTimeEnd = new Time(
							selectedTimeStart.getHour() + 1, 0, 0); // fix the
																	// corresponding
																	// timeEnd;
					Date selectedDate = dateInAWeekList.get(i - 1); // find the
																	// selected
																	// date;
					BookedDateTime selectedDateTime = new BookedDateTime(
				...
				/* deselect a chosen time slot and unmark the cell */
					...
					weekViewTable.deselectAll();
					Time deselectedTimeStart = new Time(Time.parseHour(item
							.getText(0)), 0, 0); // find the deselected
													// timeStart
					Time deselectedTimeEnd = new Time(
							deselectedTimeStart.getHour() + 1, 0, 0); // fix the
																		// corresponding
																		// timeEnd;
					Date deselectedDate = dateInAWeekList.get(i - 1); // find
																		// the
																		// deselected
																		// date;
					BookedDateTime deselectedDateTime = new BookedDateTime(
							deselectedDate, deselectedTimeStart,
							deselectedTimeEnd);
					selectedDateTimeList.remove(deselectedDateTime.toString());
					for (int j = 0; j < bookedDateTimeList.size(); j++) {
						if (bookedDateTimeList.get(j).isEqualTo(
								deselectedDateTime)) {
					...
					weekViewTable.deselectAll(); // clear the redundant
													// selection
			}
		}
	}
	...
	public class clear extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			...
			/*
			 * update the bookedDateTimelist to be passed to the venue booking
			 * page
			 */
	...
	public class goBooking extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			if (selectedDateTimeList.getItemCount() != 0) { // at least one time
															// slot is selected
				Shell vBookingShell = new Shell(getDisplay());
				Image icon = new Image(getDisplay(), "resources/eManager.png");
				vBookingShell.setText("eManager");
				vBookingShell.setImage(icon);
				VenueBooking_VenueBooking vBookingPage = new VenueBooking_VenueBooking(
						vBookingShell, SWT.None, selectedVenue,
						bookedDateTimeList);
				getParent().dispose();
				vBookingPage.pack();
				vBookingShell.pack();

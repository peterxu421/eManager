...
public class AddInflowPage extends Composite {
	...
	public AddInflowPage(Composite parent, int style, Event event) {
		...
		if (index >= 0
				&& index < EventPlanning_Budget.InflowTable.getItemCount())
			sponsor.setText(EventPlanning_Budget.InflowTable.getItem(index)
					.getText(0));
		...
		if (index >= 0
				&& index < EventPlanning_Budget.InflowTable.getItemCount())
			amount.setText(EventPlanning_Budget.InflowTable.getItem(index)
					.getText(1));
		...
		if (index >= 0
				&& index < EventPlanning_Budget.InflowTable.getItemCount()) {
			Date dt = Date.parseDate(EventPlanning_Budget.InflowTable.getItem(
					index).getText(2));
		...
		if (index >= 0
				&& index < EventPlanning_Budget.InflowTable.getItemCount())
			remarks.setText(EventPlanning_Budget.InflowTable.getItem(index)
					.getText(3));
		...
		public void widgetSelected(SelectionEvent e) {
			...
			_date = String.format("%04d", date.getYear()) + "-"
					...
					+ String.format("%02d", date.getDay()); // getMonth() + 1
															// since getMonth()
															// returns 0 to 11
			if (!sponsor.getText().isEmpty() && !amount.getText().isEmpty()
					&& !remarks.getText().isEmpty()) {
				if (localIndex == -1) {
					TableItem temp = new TableItem(
							EventPlanning_Budget.InflowTable, SWT.NULL);
					...
					double currentAmount = Double
							.parseDouble(EventPlanning_Budget.lblReceivedAmount
									.getText());
					EventPlanning_Budget.lblReceivedAmount.setText(String
							.valueOf(df.format(currentAmount + _amount))); // the
																			// double
																			// value
																			// is
																			// formated
					...
					Inflow newInflow = new Inflow(_sponsor, _amount,
							Date.parseDate(_date), _remarks);
					...
					TableItem temp = EventPlanning_Budget.InflowTable
							.getItem(localIndex);
					double currentAmount = Double
							.parseDouble(EventPlanning_Budget.lblReceivedAmount
									.getText());
					...
					temp.setText(3, _remarks);
					EventPlanning_Budget.lblReceivedAmount.setText(String
							.valueOf(df.format(currentAmount + _amount))); // add
																			// the
																			// new
																			// value

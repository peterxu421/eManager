...
public class AddBudgetAllocationPage extends Composite {
	...
	private Event event;
	private DecimalFormat df = new DecimalFormat("#.##"); // to format a double value to two decimal places
	...
	public AddBudgetAllocationPage(Composite parent, int style, Event event) {
		...
		if(index >=0 && index < EventPlanning_Budget.AllocationTable.getItemCount())
			item.setText(EventPlanning_Budget.AllocationTable.getItem(index).getText(0));
		...
		if(index >=0 && index < EventPlanning_Budget.AllocationTable.getItemCount())
			personInCharge.setText(EventPlanning_Budget.AllocationTable.getItem(index).getText(1));
		...
		if(index >=0 && index < EventPlanning_Budget.AllocationTable.getItemCount())
			cost.setText(EventPlanning_Budget.AllocationTable.getItem(index).getText(2));
	...
	class Add extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			...
			if (!item.getText().isEmpty()
				...
				if(localIndex ==-1){
					TableItem temp = new TableItem(EventPlanning_Budget.AllocationTable, SWT.NULL);
					...
					EventPlanning_Budget.lblYouStillHave_Amount.setText(String.valueOf(df.format(currentAmount - _cost)));
					
					/* update database */
					DatabaseReader db = new DatabaseReader();
					BudgetAllocation newAllocation = new BudgetAllocation(_item, _personInCharge, _cost, Date.parseDate(_date));
					db.insertBudgetAllocation(event, newAllocation);
					
					getParent().dispose();
				}
				else {
					TableItem temp = EventPlanning_Budget.AllocationTable.getItem(localIndex);
					double currentAmount = Double.parseDouble(EventPlanning_Budget.lblYouStillHave_Amount.getText());
					...
					temp.setText(2, String.valueOf(_cost));
					temp.setText(3, _date);
					EventPlanning_Budget.lblYouStillHave_Amount.setText(String.valueOf(df.format(currentAmount - _cost))); // deduct the new value
					
					/* update database */
					DatabaseReader db = new DatabaseReader();
					BudgetAllocation newAllocation = db.getBudgetAllocation(event).get(localIndex);

...
public class AddOutflowPage extends Composite {
	...
	public AddOutflowPage(Composite parent, int style, Event event) {
		...
		if(index >=0 && index < EventPlanning_Budget.OutflowTable.getItemCount())
			item.setText(EventPlanning_Budget.OutflowTable.getItem(index).getText(0));
		
		quantity = new Text(composite, SWT.BORDER);
		quantity.setBounds(253, 108, 123, 21);
		toolkit.adapt(quantity, true, true);
		if(index >=0 && index < EventPlanning_Budget.OutflowTable.getItemCount())
			quantity.setText(EventPlanning_Budget.OutflowTable.getItem(index).getText(1));
		
		type = new Text(composite, SWT.BORDER);
		type.setBounds(253, 143, 123, 21);
		toolkit.adapt(type, true, true);
		if(index >=0 && index < EventPlanning_Budget.OutflowTable.getItemCount())
			type.setText(EventPlanning_Budget.OutflowTable.getItem(index).getText(2));
		...
		if(index >=0 && index < EventPlanning_Budget.OutflowTable.getItemCount())
			cost.setText(EventPlanning_Budget.OutflowTable.getItem(index).getText(4));
	...
	class Add extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			...
			if (!item.getText().isEmpty()
				...
				if(localIndex ==-1){
					TableItem temp = new TableItem(EventPlanning_Budget.OutflowTable, SWT.NULL);
					...
					EventPlanning_Budget.lblSpentAmount.setText(String.valueOf(df.format(currentAmount + _cost)));
					
					/* update database */
					DatabaseReader db = new DatabaseReader();
					Outflow newOutflow = new Outflow(_item, _quantity, _type, Date.parseDate(_date), _cost);
					...
					TableItem temp = EventPlanning_Budget.OutflowTable.getItem(localIndex);
					double currentAmount = Double.parseDouble(EventPlanning_Budget.lblSpentAmount.getText());
					...
					temp.setText(4, String.valueOf(_cost));
					EventPlanning_Budget.lblSpentAmount.setText(String.valueOf(df.format(currentAmount + _cost))); // add the new value
